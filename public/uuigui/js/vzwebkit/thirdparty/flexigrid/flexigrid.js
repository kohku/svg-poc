...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			query: '',
			qtype: '',
			nomsg: 'No order(s) found for the search criteria',
		...
		var g = {
			...
			rePosDrag: function () {
				...
				var cdcounter=0;
				$('div', g.cDrag).hide();
				$('thead tr:first th:visible', this.hDiv).each(function (index,obj) {
                                        var n = index;
                                        var left_offset = $(obj).position().left-1;
					var cdpos = parseInt($('div', this).outerWidth());
                                        cdpos += left_offset;
					if (isNaN(cdpos)) {
						cdpos = 0;
					}
			...
			changeSort: function (th) { //change sortorder
				...
				} else if (p.sortJS || !p.url) {
                                    var tds = $(this.bDiv).find("[abbr='"+p.sortname+"']");
			...
			populate: function () { //get latest data
				...
				$.ajax({
					...
					success: function (data) {
						g.addData(data);
                                                if (p.afterAjaxCall) p.afterAjaxCall(data);
					},
